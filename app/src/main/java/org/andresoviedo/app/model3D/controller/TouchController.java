...
public class TouchController implements Handler.Callback{

    private static final String TAG = TouchController.class.getName();
    private static final String TAG_VECTORING_ROTATE = TouchController.class.getName()+"VECTORING";
    ...
    //object到屏幕的距离 限制不让object移出屏幕边界
    private float lastObjectDistance = 5;
    private PropertyValuesHolder positivePropertyValuesHolder;

    private Handler handler;

    private boolean objectRotate = false;
    private static final int HANLDER_OBJECT_ROTATE = 1;


    private final float VECTOR_SCALELING = 0.05f;
    private final float VECTOR_ROTATEING = 0.003f;
    ...
    public TouchController(ModelSurfaceView view, ModelRenderer renderer) {
        ...
        view.addOnLayoutChangeListener(new View.OnLayoutChangeListener() {
            ...
            public void onLayoutChange(View v, int left, int top, int right, int bottom, int oldLeft, int oldTop, int oldRight, int oldBottom) {
                TouchController.this.halfScreenWidth = v.getWidth() / 2;
                TouchController.this.halfScreenHeight = v.getHeight() / 2;
                Log.e(TAG,"onLayoutChange");
            }
        });

        positivePropertyValuesHolder = PropertyValuesHolder.ofFloat("null", VECTOR_SCALELING);
        handler = new Handler(Looper.getMainLooper(), this);
        handler.sendEmptyMessageDelayed(HANLDER_OBJECT_ROTATE, 100);

    }



    private class MoveCameraZRunnable implements Runnable {
        float value;

        public MoveCameraZRunnable(float value) {
            this.value = value;
        }

        @Override
        public void run() {
            mRenderer.getCamera().MoveCameraZ(value);
        }
    ...
    public synchronized boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        switch (motionEvent.getActionMasked()) {
            case MotionEvent.ACTION_UP:
                ...
                if (lastObjectDistance >= 5.5f)
                    expandObject();
                else if (lastObjectDistance <= 1.5f)
                    zoomObject();
        ...
        if (touchDelay > 1) {
            ...
            if (pointerCount == 1 && currentPress1 > 4.0f) {
                ...
                mRenderer.getCamera().translateCamera(dx1, 0);
                lastObjectDistance = 5;
            } else if (pointerCount == 2) {
                //if (fingersAreClosing) {

                float[] nearPoints = unproject(halfScreenWidth, halfScreenHeight, 0);
                float[] farPoints = unproject(halfScreenWidth, halfScreenHeight, 1);

                lastObjectDistance = selectObjectImpl(nearPoints, farPoints);

                if (lastObjectDistance >= 5.5f || lastObjectDistance <= 1.5f) return true;
     ...
     */
    private float selectObjectImpl(float[] nearPoint, float[] farPoint) {
        SceneLoader scene = view.getModelActivity().getScene();
        if (scene == null) {
            return -10;
        }
        Object3DData obj = scene.getObjects().get(0);
        return Math3DUtils.calculateDistanceOfIntersection(nearPoint, farPoint, obj.getPosition(), 1f);
    }

    private float[] unproject(float rx, float ry, float rz) {
        float[] xyzw = {0, 0, 0, 0};
    ...
    private void expandObject() {
        ValueAnimator pvalue = ValueAnimator.ofPropertyValuesHolder(positivePropertyValuesHolder);
        pvalue.setDuration(1600);
        pvalue.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                Float f = Float.valueOf(animation.getAnimatedValue() + "");
                mRenderer.getCamera().MoveCameraZ(f);
                Log.e(TAG_VECTORING_ROTATE, "expanding");
            }
        });
        pvalue.start();
    }
    private void zoomObject() {
        ValueAnimator pvalue = ValueAnimator.ofPropertyValuesHolder(positivePropertyValuesHolder);
        pvalue.setDuration(800);
        pvalue.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                Float f = Float.valueOf(animation.getAnimatedValue() + "");
                mRenderer.getCamera().MoveCameraZ(-f);
                Log.e(TAG_VECTORING_ROTATE, "zooming");
            }
        });
        pvalue.start();
    }
    ...
    public boolean handleMessage(Message msg) {
        ...
        switch (msg.what) {
            case HANLDER_OBJECT_ROTATE:

                handler.sendEmptyMessageDelayed(HANLDER_OBJECT_ROTATE, 20);
                if (objectRotate && mRenderer != null && mRenderer.getCamera() != null)
                    mRenderer.getCamera().translateCamera(-VECTOR_ROTATEING, 0);
